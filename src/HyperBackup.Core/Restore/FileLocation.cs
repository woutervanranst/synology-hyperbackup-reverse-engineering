...
/// content reads). Used both to restore the file and to decide which blobs must be
/// rehydrated from Archive before a restore.
/// </summary>
public sealed record FileLocation(
    StorageKind Kind,
    IReadOnlyList<string> BlobPaths,
    IReadOnlyList<ChunkLocation> Chunks,
    long? FilePoolId,
    string? Note)
{
